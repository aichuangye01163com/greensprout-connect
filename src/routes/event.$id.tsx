  ...
  head: () => ({
    meta: [
      { title: "活动详情 · 绿芽局 GreenSprout" },
      {
        name: "description",
        content:
          "查看活动日程、地点、参与者与报名条件，一键报名同城小局。",
      },
      {
        property: "og:title",
        content: "活动详情 · 绿芽局 GreenSprout",
      },
      {
        property: "og:description",
        content: "查看日程、参与者与报名条件，一键报名。",
      },
    ],
  }),
...
function EventDetail() {
   ...
   *
   * 身份依据：
   * Supabase Auth UUID === activities.host_id
   *
   * 不再使用昵称判断。
   */
  const isHost =
  ...
  const loadPendingMembers = async () => {
    ...
    try {
      const members = await getPendingMembers(event.id);
      setPendingMembers(members);
    } catch (error) {
      console.error("获取待审核报名失败:", error);
      toast.error("获取待审核报名失败");
    } finally {
   ...
   * pending → approved
   *
   * 数据库 trigger 会自动同步到 chat_members。
   */
  const handleApprove = async (
    member: PendingActivityMember
  ) => {
    ...
    try {
      ...
      toast.success(
        `${member.profile?.nickname ?? "该用户"} 已通过审核`
      );

      await loadPendingMembers();

      // 刷新当前活动群状态。
      // chat_members 的实际同步由数据库 trigger 完成。
      setRoom(await getChatRoom(event.id));
    } catch (error) {
      ...
      <div className="space-y-6 pb-24">
            ...
            <div className="flex items-start gap-2 rounded-2xl border border-border bg-card p-4 text-sm">
              ...
              <p className="min-w-0 text-muted-foreground">
                {viaInvite
                  ? "你通过定向邀请链接进入这场私密活动室。"
                  : "这是一场私密活动室。"}
                点击「报名加入」并输入活动室密码，验证通过后提交报名申请，等待组织者审核。
              </p>
            </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card">
          ...
          <div className="space-y-3 p-4">
            ...
            <h1 className="text-lg leading-snug tracking-tight">
              {event.title}
            </h1>
            ...
            {pendingLoading ? (
              ...
              <ul className="space-y-3">
                {pendingMembers.map((member) => (
                  ...
                  >
                    <span className="grid size-10 shrink-0 place-items-center rounded-full bg-secondary text-base">
                      {member.profile?.avatarUrl ?? "🌱"}
                    </span>

                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm">
                        {member.profile?.nickname ?? "用户"}
                      </p>
        ...
        <Section
          title={`参与者 ${event.joined}/${event.limit}`}
        >
          ...
          <Section title="临时群聊">
            {isPrivate && !joined && !isHost ? (
              <p className="flex items-center gap-2 text-sm text-muted-foreground">
                <Lock className="size-4" /> 私密活动室，输入密码提交报名申请后，等待组织者审核
              </p>
            ) : !room?.unlocked ? (
              <p className="flex items-center gap-2 text-sm text-muted-foreground">
      ...
      >
        <DialogContent className="max-w-sm rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              这是一场私密活动室，请输入组织者发给你的密码。验证通过后将提交报名申请，等待组织者审核。
            </DialogDescription>
          </DialogHeader>
...
function Section({
  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-4">
      <h2 className="mb-3 text-sm tracking-wide text-muted-foreground">
        {title}
      </h2>
